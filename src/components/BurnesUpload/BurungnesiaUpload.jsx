...
function BurungnesiaUpload() {
    ...
    const [formData, setFormData] = useState({
        ...
        images: [],
        sounds: [],
        label: '', // Nama lokasi untuk checklists
        habitat: '' // Deskripsi habitat untuk checklists
    });
 ...
 const handleFaunaNameChange = async (name) => {
    ...
    if (name.length > 2) {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/taksa/search/burungnesia?q=${encodeURIComponent(name)}&limit=15`, {
                headers: {
            ...
            if (data.status === 'success' && data.data && data.data.length > 0) {
                const formattedResults = data.data.map(item => ({
                    id: item.id,
                    nameId: item.common_name || item.scientific_name,
                    nameLat: item.scientific_name,
                    displayName: item.common_name || item.scientific_name,
                    taxa_id: item.id,
                    taxon_rank: item.rank
                }));
                const sortedResults = sortSearchResults(formattedResults, name);
...
const handleInputChange = (e) => {
 ...
 const handleLocationSave = (lat, lng, name) => {
     setFormData(prevFormData => {
         const labelValue = prevFormData.label || name || '';
         return {
             ...prevFormData,
             latitude: lat,
             longitude: lng,
    ...
    const handleEditBird = async (index) => {
        ...
        setFaunaName(bird.faunaName);
            ...
            setFormData(prevData => ({
                ...
                breeding_type_id: Array.isArray(bird.breeding_type_id) ? [...bird.breeding_type_id] : (bird.breeding_type_id ? [bird.breeding_type_id] : []),
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            birdList.forEach((bird, index) => {
                ...
                if (bird.breeding && Array.isArray(bird.breeding_type_id) && bird.breeding_type_id.length > 0) {
                    bird.breeding_type_id.forEach(typeId => {
                        formDataToSend.append(`breeding_type_id[${index}][]`, typeId);
                    });
                }
            ...
            const response = await apiFetch('/burungnesia/checklist-fauna', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                try {
                    ...
                    console.error('Gagal update total observasi:', e);
                }
                
                toast.success('Data berhasil diunggah! Mengalihkan ke halaman observasi...');
                setFormData({
                    latitude: '',
                    longitude: '',
                    tujuan_pengamatan: '',
                    ...
                    <div className="bg-[#3a0f0f] border border-red-700 text-red-300 px-4 py-3 rounded relative mb-4">
                        ...
                        <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
                            <svg onClick={() => setError('')} className="fill-current h-6 w-6 text-red-400" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <title>Close</title>
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/>
                            </svg>
                        </span>
                    </div>
                )}
<form onSubmit={handleConfirmSubmit} className="space-y-6">
    ...
    <div className="bg-[#1e1e1e] p-4 rounded-lg border border-[#444]">
        ...
        <LocationInput
            ...
            onLabelChange={(val) => setFormData(prev => ({ ...prev, label: val }))}
    ...
    {/* Informasi Lokasi */}
    <div className="bg-[#1e1e1e] p-4 rounded-lg border border-[#444]">
        <h3 className="text-lg font-medium mb-3 text-white">Informasi Lokasi</h3>
        <div className="grid grid-cols-1 gap-4">
            <div className="space-y-2">
                <label htmlFor="habitat" className="block text-sm text-gray-300">
                    Deskripsi Habitat
                </label>
                <textarea 
                    id="habitat"
                    ...
                    onChange={handleInputChange}
                    placeholder="Contoh: Hutan hujan tropis dataran rendah, ketinggian 500-800 mdpl"
                    rows="3"
                />
            </div>
        </div>
                ...
                <div className="max-h-40 overflow-y-auto mt-2 pr-2">
                    {birdList.map((bird, index) => (
                        <div key={index} className="mt-2 border-t border-[#444] pt-2">
                            ...
                            {bird.breeding && Array.isArray(bird.breeding_type_id) && bird.breeding_type_id.length > 0 && (
                                <p><strong className="text-white">Tipe Breeding:</strong> <span className="text-gray-300">
                                    {bird.breeding_type_id.map(id => {
                                        const types = {1:'Menarik pasangan',2:'Kawin',3:'Mengumpulkan material sarang',4:'Membangun sarang',5:'Mengeram',6:'Memberi makan anakan',7:'Memberi makan induk dalam sarang',8:'Memberi makan anakan di luar sarang'};
                                        return types[id] || '';
                                    }).filter(Boolean).join(', ')}
                                </span></p>
                            )}
                                ...
                                <td className="px-6 py-4 text-sm text-gray-300">
                                    {bird.breeding ? 
                                        <div>
                                            ...
                                            {Array.isArray(bird.breeding_type_id) && bird.breeding_type_id.length > 0 && (
            ...
            <div className="space-y-2">
                    ...
                    name="breeding" 
                        ...
                        onChange={(e) => {
                            setFormData(prev => ({
                                ...
                                breeding_type_id: e.target.checked ? prev.breeding_type_id : []
                            }));
                        }}
                ...
                {formData.breeding && (
                    <div className="mt-3 bg-[#2c2c2c] p-3 rounded-lg">
                        <p className="text-sm font-medium text-gray-300 mb-2">Pilih tipe breeding (bisa pilih lebih dari satu):</p>
                        <div className="space-y-2">
                            {[
                                { id: 1, label: 'Menarik pasangan' },
                                { id: 2, label: 'Kawin' },
                                { id: 3, label: 'Mengumpulkan material sarang' },
                                { id: 4, label: 'Membangun sarang' },
                                { id: 5, label: 'Mengeram' },
                                { id: 6, label: 'Memberi makan anakan' },
                                { id: 7, label: 'Memberi makan induk dalam sarang' },
                                { id: 8, label: 'Memberi makan anakan di luar sarang' },
                            ].map(type => (
                                <div key={type.id} className="flex items-center space-x-2">
                                    <input
                                        type="checkbox"
                                        id={`breeding_type_${type.id}`}
                                        checked={Array.isArray(formData.breeding_type_id) && formData.breeding_type_id.includes(type.id)}
                                        onChange={() => setFormData(prev => {
                                            const current = Array.isArray(prev.breeding_type_id) ? prev.breeding_type_id : [];
                                            const updated = current.includes(type.id)
                                                ? current.filter(id => id !== type.id)
                                                : [...current, type.id];
                                            return {...prev, breeding_type_id: updated};
                                        })}
                                        className="w-4 h-4 text-[#1a73e8] rounded focus:ring-[#1a73e8] accent-[#1a73e8]"
                                    />
                                    <label htmlFor={`breeding_type_${type.id}`} className="text-gray-300">{type.label}</label>
                                </div>
                            ))}
                        </div>
...
export default BurungnesiaUpload;