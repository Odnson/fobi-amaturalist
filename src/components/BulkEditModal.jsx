...
// License options (same as MediaCard)
...
function BulkEditModal({ isOpen, onClose, onSave, selectedItems }) {
    ...
    // Tambahkan state untuk menyimpan data sementara saat modal lokasi dibuka
    ...
    const renderTaxonSuggestions = (searchResults) => {
        // Sorting: by scientific_name asc, then by rank precedence (older/higher first)
        const rankOrder = {
            family: 100,
            subfamily: 90,
            tribe: 85,
        ...
        // Use Map to filter duplicates by scientific_name + rank before building hierarchy
        ...
        // Build hierarchical order: family → subfamily → tribe → subtribe → genus → subgenus → species → subspecies
        ...
        nodes.forEach(parent => {
            nodes.forEach(child => {
                ...
                // Avoid linking genus -> subspecies when a matching species exists; let species be direct parent
        ...
        // Reparent pass: ensure subspecies are placed under their species when present
        ...
        // Build a list of searchable strings from taxon and its full_data (mirrors MediaCard.jsx)
    ...
    const handleSuggestionClick = (suggestion) => {
        ...
        // Reset semua data taksonomi terlebih dahulu
        const resetData = {
            ...tempFormData,
            taxon_id: '',
            species: '',
    ...
    // Reset form ketika modal ditutup dan hapus data dari localStorage
    useEffect(() => {
        if (!isOpen) {
            setTempFormData({
                scientific_name: '',
            ...
            // Hapus data dari localStorage saat modal ditutup
            localStorage.removeItem('bulk_edit_temp_data');
            
            // Reset tempDataRef
            tempDataRef.current = null;
        } else if (isOpen && selectedItems.length > 0) {
            // Initialize form with empty values for bulk editing
            // User can fill only the fields they want to update
            setTempFormData({
                scientific_name: '',
                habitat: '',
                description: '',
    ...
    // Simpan state saat ini sebelum membuka modal lokasi
    ...
    const handleSave = () => {
        if (validateForm()) {
            ...
            // Hapus data dari localStorage saat menyimpan
            localStorage.removeItem('bulk_edit_temp_data');
            
            // Tambahkan flag ke dataToSave untuk menunjukkan bahwa ini adalah bulk edit
            const finalData = {
                ...dataToSave,
                __isBulkEdit: true
            };
    ...
    const handleLocationSave = (lat, lng, name) => {
        // Gunakan data dari tempDataRef untuk memastikan tidak ada data yang hilang
    ...
    // Tambahkan handleInputFocus untuk memastikan suggestions muncul kembali saat fokus pada input
    ...
    // Tambahkan cleanup pada unmount
    useEffect(() => {
        return () => {
            if (timeoutRef.current) {
                clearTimeout(timeoutRef.current);
            ...
            // Hapus data dari localStorage saat komponen unmount
            localStorage.removeItem('bulk_edit_temp_data');
        };
    }, []);
