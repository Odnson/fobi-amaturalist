...
function ImageComments({ images = [], onRatingUpdate, source }) {
    ...
    const getFullImageUrl = (url) => {
        ...
        return 'https://kupunesia.org/storage/images_burung/' + url;
    };
    ...
    const fetchImageData = async (imageId) => {
        try {
            ...
            if (commentsData.success && ratingsData.success) {
                ...
                if (ratingsData.data?.user_rating != null && !storedRating) {
                    localStorage.setItem(`media_rating_${imageId}`, String(ratingsData.data.user_rating));
                    userRating = ratingsData.data.user_rating;
                }
                
                return {
                    imageId,
                    imageUrl: getFullImageUrl(imageInfo?.url || imageInfo?.filename || ''),
