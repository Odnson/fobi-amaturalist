...
function MediaViewer({ checklistData, images = [], sounds = [], onRatingUpdate, source }) {
    ...
    const getFullImageUrl = (url) => {
        ...
        const baseUrl = 'https://kupunesia.org/storage/images_burung/';
        return baseUrl + url;
    };
    const processedImages = React.useMemo(() => {
        return (images || []).map(img => ({
            ...img,
            url: getFullImageUrl(img.url || img.filename)
        }));
    }, [images]);
    const groupedMedia = React.useMemo(() => {
        return (checklistData?.fauna || []).reduce((acc, fauna) => {
            const faunaIdStr = String(fauna.id);
            const faunaImages = processedImages.filter(img => String(img.fauna_id) === faunaIdStr);
            acc[fauna.id] = {
                fauna,
                images: faunaImages,
                sounds: sounds.filter(sound => String(sound.fauna_id) === faunaIdStr)
            };
            return acc;
        }, {});
    }, [checklistData?.fauna, processedImages, sounds]);
    useEffect(() => {
            ...
            if (data.success) {
                ...
                if (!storedRating) {
                    if (data.data.user_rating != null) {
                        ...
                        localStorage.setItem(`media_rating_${imageId}`, String(data.data.user_rating));
                    } else {
                        setUserRating(0);
    ...
    const unmatchedImages = processedImages.filter(img => {
        const matched = (checklistData?.fauna || []).some(f => String(f.id) === String(img.fauna_id));
        return !matched;
    });
                            ...
                            <tr key={`fauna-${fauna.id}`} className="hover:bg-[#2c2c2c]">
                                <td className="px-4 py-2">
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            {fauna.nama_lokal && (
                                                <div className="font-medium text-[#e0e0e0]">{fauna.nama_lokal}</div>
                                            )}
                                            {fauna.nama_ilmiah && (
                                                <div className={`text-sm ${fauna.nama_lokal ? 'text-[#b0b0b0]' : 'text-[#e0e0e0] font-medium'} italic`}>{fauna.nama_ilmiah}</div>
                                            )}
                                            {!fauna.nama_lokal && !fauna.nama_ilmiah && fauna.family && (
                                                <div className="font-medium text-[#e0e0e0]">{fauna.family}</div>
                                            )}
                                        </div>
