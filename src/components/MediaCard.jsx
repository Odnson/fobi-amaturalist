...
// License options (must match BulkEditModal)
...
// Konstanta untuk sessionStorage
...
function MediaCard({ observation, isSelected, onSelect, onUpdate, onDelete, bulkFormData, qualityGrade, uploadSessionId, validationErrors, id, handleCombinedImageCrop, userLicenseDefaults }) {
    ...
    const [formData, setFormData] = useState({
        ...
        // License fields with defaults from user preferences or fallback
    ...
    useEffect(() => {
        // Only apply bulkFormData if this card is currently selected AND bulkFormData exists
        // CRITICAL FIX: Check if bulkFormData has __isBulkEdit flag to ensure it's from a legitimate bulk edit
        if (isSelected && bulkFormData && Object.keys(bulkFormData).length > 0 && bulkFormData.__isBulkEdit) {
            // CRITICAL: Define media-related fields that should NEVER be overwritten during bulk edit
            const mediaFields = [
                'file', 'files', 'audioFiles', 'imageFiles', 'spectrogramUrl', 'spectrogramUrls',
                'isCombined', 'combinedOrder', 'type', 'id'
            ];
            
            // Jika bulkFormData memiliki scientific_name yang tidak kosong, reset data taksonomi
            if (bulkFormData.scientific_name) {
                setFormData(prev => {
                    // Reset data taksonomi terlebih dahulu
                    const resetData = {
                        ...prev,
                        taxon_id: '',
                        species: '',
                    ...
                    // Kemudian isi dengan data baru dari bulkFormData, KECUALI media fields
                    return {
                        ...resetData,
                        ...Object.fromEntries(
                ...
                // Untuk field lain selain taksonomi, hanya update jika ada di bulkFormData
                setFormData(prev => ({
                    ...prev,
                    ...Object.fromEntries(
                        Object.entries(bulkFormData)
    ...
    // Load suggestions from sessionStorage when component mounts
    useEffect(() => {
        try {
            ...
            if (storedSuggestions && storedQuery) {
                // Hanya pulihkan saran jika query cocok dengan scientific_name saat ini
                if (storedQuery === formData.scientific_name) {
                    ...
                    // Tampilkan saran jika ada dan panjang query > 2
    ...
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible' && formData.scientific_name.length > 2) {
                // When tab becomes visible again, show suggestions if they exist
                try {
                    const storedSuggestions = sessionStorage.getItem(SUGGESTIONS_STORAGE_KEY);
                    const storedQuery = sessionStorage.getItem(SUGGESTIONS_QUERY_KEY);
    ...
    const handleInputChange = async (e) => {
        ...
        // If changing license on a combined observation, rebuild fileLicenseMapping
        if (name.startsWith('license_') && observation?.isCombined) {
            ...
            // Determine counts for audio/image files, support either dedicated arrays or generic files[]
        ...
        // Propagate license changes immediately to parent component
        ...
        if (name === 'scientific_name') {
            ...
            if (value.toLowerCase() === 'unknown') {
                ...
                // Hapus data saran dari sessionStorage jika input kosong
                try {
            ...
            // Cek apakah ada saran tersimpan untuk query ini
            try {
                const storedSuggestions = sessionStorage.getItem(SUGGESTIONS_STORAGE_KEY);
                const storedQuery = sessionStorage.getItem(SUGGESTIONS_QUERY_KEY);
                ...
                if (value.length > 2) {
                    try {
                        ...
                        if (data.success) {
                            ...
                            // Simpan saran ke sessionStorage
                            try {
                            ...
                            // Hapus saran dari sessionStorage jika tidak ada hasil
                            try {
                                sessionStorage.removeItem(SUGGESTIONS_STORAGE_KEY);
                                sessionStorage.removeItem(SUGGESTIONS_QUERY_KEY);
                                sessionStorage.removeItem(SUGGESTIONS_PAGE_KEY);
        ...
        try {
            ...
            if (data.success) {
                ...
                if (uniqueNewSuggestions.length > 0) {
                    ...
                    // Update saran di sessionStorage
                    try {
                    ...
                    // Update status hasMore di sessionStorage
                    try {
    ...
    // Function to normalize scientific names by removing author citations
    const normalizeScientificName = (scientificName) => {
        if (!scientificName) return scientificName;
        
        // Remove author citations and years in parentheses
        let normalized = scientificName
            // Remove parentheses with author and year: (Author, Year) or (Author Year)
            .replace(/\s*\([^)]*\d{4}[^)]*\)/g, '')
            // Remove complex multi-author citations with year
            .replace(/\s+[A-Z][a-zA-Z]*(?:\s*,\s*[A-Z][a-zA-Z]*)*(?:\s*&\s*[A-Z][a-zA-Z]*)?\s*,\s*\d{4}.*$/g, '')
            // Remove author citations with initials and year
            .replace(/\s+[A-Z](?:\.[A-Z])*\.?\s+[A-Z][a-zA-Z]*(?:\s*,\s*\d{4}.*)?$/g, '')
            // Remove author citations with year
            .replace(/\s+[A-Z][a-zA-Z]*(?:\s*&\s*[A-Z][a-zA-Z]*)*(?:\s*,\s*\d{4}.*)?$/g, '')
            // Remove author citations without year but with comma
            .replace(/\s+[A-Z][a-zA-Z]*(?:\s*&\s*[A-Z][a-zA-Z]*)*,\s*$/g, '')
            // Remove single author abbreviations: L., Mill., etc.
            .replace(/\s+[A-Z][a-z]*\.\s*$/g, '')
            // Remove remaining author initials: G.R., A.B.C., etc.
            .replace(/\s+[A-Z](?:\.[A-Z])*\.?\s*$/g, '')
    ...
    const renderTaxonSuggestions = (searchResults) => {
        // Skip frontend sorting for specific queries where backend ordering should be preserved
        ...
        if (shouldPreserveBackendOrder) {
            // Preserve backend ordering for these specific queries
            processedResults = [...(searchResults || [])];
        } else {
            // Apply frontend alphabetical sorting for other queries
        ...
        // Complete taxonomic hierarchy matching StatsBar.jsx - ALL 27 ranks
        const rankOrder = {
            'form': 1,
            'variety': 2,
            'subspecies': 3,
        ...
        const getBinomial = (name) => {
            ...
            for (let i = 0; i < parts.length; i++) {
                const token = parts[i];
                if (!genus) {
                    // pick first token starting with uppercase letter (genus)
                    if (/^[A-Z][a-zA-Z-]*$/.test(token)) {
                        genus = token;
                    }
                } else {
                    // pick next token that looks like species epithet (lowercase, no parentheses/digits)
                    if (/^[a-z-]+$/.test(token)) {
                        species = token;
                        break;
                    }
        ...
        const isParentOf = (parentTaxon, childTaxon) => {
            ...
            // Direct lineage checks
            if (pr === 'family') {
                // parent family matches child's family
            ...
            if (pr === 'genus') {
                // genus parent of subgenus/species/subspecies
                return p.genus && ((c.genus && p.genus === c.genus) || (c.subgenus_parent && p.genus === c.subgenus_parent));
            }
            if (pr === 'subgenus') {
                // subgenus parent of species/subspecies within same subgenus
                return p.subgenus && c.subgenus && p.subgenus === c.subgenus;
            }
            if (pr === 'species') {
                // species parent of subspecies (fallback to binomial compare)
        ...
        // Link direct parents and children
        nodes.forEach(parent => {
            nodes.forEach(child => {
                if (parent === child) return;
                if (!isParentOf(parent, child)) return;
                // Special case: avoid linking genus -> subspecies if corresponding species exists
                ...
                // Ensure direct parent (no intermediate)
        ...
        // Reparent pass: ensure every subspecies sits under its species if the species exists anywhere
        ...
        // helper to remove a child from any parent's children list
        ...
        // Get query first for prioritization
        const query = (formData?.displayName || formData?.scientific_name || '').trim().toLowerCase();
        
        // Prioritization per requirements:
        // 1) Exact matches go to the top with their children
        // 2) Then show ALL taxa containing keyword, sorted by evolutionary age (oldest first) then alphabetically
        // 3) Then show remaining taxa, sorted by evolutionary age then alphabetically
        // 4) Case-insensitive matching anywhere in the name
        let prioritizedList = hierarchicalResults;

        if (query && hierarchicalResults.length) {
            // Build a list of searchable strings from taxon and its full_data
            ...
            // Exact match: any searchable string equals query
            const nameEquals = (t) => {
                const vals = getSearchableStrings(t);
                return vals.some(v => v === query);
            };

            // Contains match: any searchable string contains query
            const nameContains = (t) => {
                const vals = getSearchableStrings(t);
                return vals.some(v => v.includes(query));
            };

            // Hierarchical prioritization: maintain parent-child structure while prioritizing containing matches
            ...
            if (containingMatches.length > 0) {
                // Rank order for prioritization (higher values = older/higher rank)
                const rankOrder = {
                ...
                // Find the highest-ranking containing match
                ...
                // Find the node in the hierarchical structure that corresponds to the highest match
                const highestMatchNode = nodes.find(n => 
                    n.scientific_name === highestRankMatch.scientific_name && n.rank === highestRankMatch.rank
                );
                
                if (highestMatchNode) {
                    // Collect the highest match and all its descendants
                    const collected = [];
                    const collect = (n) => {
                        if (!n) return;
                        collected.push(n);
                        if (n.children && n.children.length) {
                            // Sort children by rank precedence then alphabetically
                    ...
                    // Get remaining items (other containing matches + non-matches)
                    ...
                    // Sort remainder: containing matches first, then non-matches, both by rank then alphabetically
                    ...
                    // Final order: highest match + descendants, then other containing matches, then non-matches
                    prioritizedList = [...collected, ...remainingContaining, ...remainingNonContaining];
                } else {
                    // Fallback: flat sort if node structure is broken
        ...
        // Debug log of suggestion results (hierarchical order)
        try {
            ...
            // Debug log final prioritized results
            if (query) {
    ...
    const handleSuggestionClick = (suggestion) => {
        // Check if this is a synonym and handle redirection to accepted name
        ...
        // If it's a synonym, use the accepted name for the scientific_name field
        const finalScientificName = isSynonym && acceptedName ? acceptedName : suggestion.scientific_name;
        
        // Reset semua data taksonomi terlebih dahulu untuk menghindari data lama tercampur
        const resetData = {
            ...formData,
            taxon_id: '',
            species: '',
        ...
        // Kemudian isi dengan data baru dari suggestion
        const updatedData = {
            ...resetData,
    ...
    const handleCropClick = (id, index) => {
        // Jika id dan index diberikan, ini adalah gambar gabungan
        if (id && index !== undefined) {
            // Gunakan prop handleCombinedImageCrop jika tersedia
            if (handleCombinedImageCrop) {
                handleCombinedImageCrop(id, index);
            }
        } else {
            // Ini adalah gambar tunggal
    ...
    const handleCropSave = (croppedFile) => {
        // Update observation dengan file yang sudah di-crop
        const updatedObservation = {
            ...observation,
            file: croppedFile
        };
        
        // Update formData untuk memastikan data form tidak hilang setelah crop
