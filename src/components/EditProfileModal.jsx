...
function EditProfileModal({ isOpen, onClose, userData, onSave }) {
    ...
    const [formData, setFormData] = useState({
        ...
        bio: userData?.bio || '',
        // Global (legacy) license; keep for backward compatibility
        license: userData?.license || '',
        // New defaults per content type (fallback to localStorage then CC BY-NC)
        license_observation: userData?.license_observation || localStorage.getItem('license_observation') || 'CC BY-NC',
        license_photo: userData?.license_photo || localStorage.getItem('license_photo') || 'CC BY-NC',
        license_audio: userData?.license_audio || localStorage.getItem('license_audio') || 'CC BY-NC',
    ...
    useEffect(() => {
        if (userData) {
            ...
            setFormData(prev => ({
                ...
                license: userData.license ?? prev.license,
                license_observation: userData.license_observation || prev.license_observation || localStorage.getItem('license_observation') || 'CC BY-NC',
                license_photo: userData.license_photo || prev.license_photo || localStorage.getItem('license_photo') || 'CC BY-NC',
                license_audio: userData.license_audio || prev.license_audio || localStorage.getItem('license_audio') || 'CC BY-NC',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                // Update localStorage with new license values
                localStorage.setItem('license_observation', formData.license_observation);
                localStorage.setItem('license_photo', formData.license_photo);
                localStorage.setItem('license_audio', formData.license_audio);
            ...
            <div className="bg-[#1e1e1e] rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto text-[#e0e0e0] border border-[#444]">
                ...
                <div className="mb-4">
                    <div className="flex border-b border-[#444]">
                        ...
                        <button
                            className={`px-4 py-2 ${activeTab === 'license' ? 'border-b-2 border-[#1a73e8] text-[#1a73e8]' : 'text-[#e0e0e0]'}`}
                ...
                {activeTab === 'license' && (
                    <form onSubmit={handleSubmit}>
                        <h3 className="text-lg font-medium mb-4 text-[#e0e0e0]">Pengaturan Lisensi</h3>
                        <p className="text-sm text-[#b0b0b0] mb-4">
                            Atur lisensi default untuk unggahan Anda. Ini hanya default; Anda masih bisa mengubah lisensi per item saat upload.
                        </p>
                        {/* Lisensi default untuk data observasi/checklist */}
                        <div className="mb-5">
                            <label className="block text-sm font-medium text-[#e0e0e0] mb-2">
                                Lisensi Data Observasi/Checklist (default)
                            </label>
                            <select
                                value={formData.license_observation}
                                onChange={(e) => setFormData({ ...formData, license_observation: e.target.value })}
                                className="w-full p-2 bg-[#2c2c2c] border border-[#444] rounded text-[#e0e0e0] focus:ring-2 focus:ring-[#1a73e8] focus:border-transparent"
                            >
                                <option value="CC0">CC0 1.0 (Public Domain)</option>
                                <option value="CC BY">CC BY 4.0 (Atribusi)</option>
                                <option value="CC BY-SA">CC BY-SA 4.0 (Atribusi- BerbagiSerupa)</option>
                                <option value="CC BY-NC">CC BY-NC 4.0 (Atribusi- NonKomersial)</option>
                                <option value="CC BY-ND">CC BY-ND 4.0 (Atribusi- TanpaTurunan)</option>
                                <option value="CC BY-NC-SA">CC BY-NC-SA 4.0 (Atribusi- NonKomersial- BerbagiSerupa)</option>
                                <option value="CC BY-NC-ND">CC BY-NC-ND 4.0 (Atribusi- NonKomersial- TanpaTurunan)</option>
                            </select>
                        </div>
                        {/* Lisensi default untuk foto */}
                        <div className="mb-5">
                            <label className="block text-sm font-medium text-[#e0e0e0] mb-2">
                                Lisensi Foto (default)
                            </label>
                            <select
                                value={formData.license_photo}
                                onChange={(e) => setFormData({ ...formData, license_photo: e.target.value })}
                                className="w-full p-2 bg-[#2c2c2c] border border-[#444] rounded text-[#e0e0e0] focus:ring-2 focus:ring-[#1a73e8] focus:border-transparent"
                            >
                                <option value="CC0">CC0 1.0 (Public Domain)</option>
                                <option value="CC BY">CC BY 4.0 (Atribusi)</option>
                        ...
                        {/* Lisensi default untuk suara/audio */}
                        <div className="mb-6">
                            <label className="block text-sm font-medium text-[#e0e0e0] mb-2">
                                Lisensi Suara/Audio (default)
                            </label>
                            <select
                                value={formData.license_audio}
                                onChange={(e) => setFormData({ ...formData, license_audio: e.target.value })}
                                className="w-full p-2 bg-[#2c2c2c] border border-[#444] rounded text-[#e0e0e0] focus:ring-2 focus:ring-[#1a73e8] focus:border-transparent"
                            >
                                <option value="CC0">CC0 1.0 (Public Domain)</option>
                                <option value="CC BY">CC BY 4.0 (Atribusi)</option>
                            ...
                            <p className="text-xs text-[#b0b0b0] mt-2">
                                Referensi lisensi: https://creativecommons.org/licenses/
                            </p>
                        </div>
                        <div className="flex justify-end gap-2">
                            <button
                ...
                {activeTab === 'email' && (
                    <div>
                        <h3 className="text-lg font-medium mb-4 text-[#e0e0e0]">Email Amaturalist</h3>
