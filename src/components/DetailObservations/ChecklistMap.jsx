...
import { MapContainer, TileLayer, Rectangle, useMap, Popup, Marker, ZoomControl, LayersControl } from 'react-leaflet';
...
function ChecklistMap({ checklist }) {
    ...
    if (!latitude || !longitude || isNaN(latitude) || isNaN(longitude)) {
        return (
            <div className="flex items-center justify-center h-[250px] sm:h-[300px] md:h-[400px] bg-[#1e1e1e] rounded-lg border border-[#444]">
                <p className="text-gray-400 text-sm sm:text-base">Lokasi tidak tersedia</p>
            </div>
        );
    }
                    ...
                    >
                        <Popup className="dark-popup">
                            <div className="text-gray-800">
                                <h4 className="font-medium">{location.scientific_name}</h4>
                                <p className="text-sm">Grade: {location.grade}</p>
                                <p className="text-gray-600 text-xs">Tanggal: {new Date(location.created_at).toLocaleDateString()}</p>
                            </div>
                        </Popup>
                    </Marker>
                ))}
