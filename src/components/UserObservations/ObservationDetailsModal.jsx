  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1e1e1e] rounded-lg shadow-lg w-full max-w-4xl border border-[#444] overflow-hidden max-h-[90vh] flex flex-col">
        ...
        {/* Footer */}
        <div className="border-t border-[#333] px-6 py-3 flex justify-between items-center">
          <div className="text-xs text-[#777]">
            ID: {details?.id} • Sumber: {details?.source || '-'}
          </div>
          {details && details.id ? (
            <button
              type="button"
              onClick={() => {
                let url = '';
                if (details.source === 'burungnesia') {
                  url = `/burungnesia/observations/${details.id}`;
                } else if (details.source === 'kupunesia') {
                  url = `/kupunesia/observations/${details.id}`;
                } else {
                  url = `/observations/${details.id}`;
                }
                window.open(url, '_blank', 'noopener,noreferrer');
              }}
              className="inline-flex items-center gap-2 px-4 py-2 bg-[#2c2c2c] text-[#e0e0e0] rounded hover:bg-[#3c3c3c] transition-colors"
            >
              <span>Buka Observasi</span>
            </button>
          ) : null}
        </div>
      </div>
