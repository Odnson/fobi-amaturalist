...
function KupunesiaUpload() {
    ...
    const [formData, setFormData] = useState({
        ...
        observer: '',
        additional_note: '',
        active: 1, // Default active = 1 agar checklist tampil
        tgl_pengamatan: '',
        start_time: '',
        end_time: '',
        completed: 0,
        ...
        breeding_note: '',
        images: [],
        label: '', // Nama lokasi untuk checklisttr
        habitat: '' // Deskripsi habitat untuk checklisttr
    });
    ...
    const handleFaunaNameChange = async (name) => {
        ...
        if (name.length > 2) {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_URL}/taksa/search/kupunesia?q=${encodeURIComponent(name)}&limit=15&kupunesia=1`, {
                    headers: {
    ...
    const handleEditButterfly = (index) => {
        ...
        setFormData(prevData => ({
            ...
            breeding_type_id: Array.isArray(butterfly.breeding_type_id) ? [...butterfly.breeding_type_id] : (butterfly.breeding_type_id ? [butterfly.breeding_type_id] : []),
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            butterflyList.forEach((butterfly, index) => {
                ...
                formDataToSend.append(`breeding[${index}]`, butterfly.breeding ? 1 : 0);
                formDataToSend.append(`breeding_note[${index}]`, butterfly.breeding_note || '');
                
                if (butterfly.breeding && Array.isArray(butterfly.breeding_type_id) && butterfly.breeding_type_id.length > 0) {
                    butterfly.breeding_type_id.forEach(typeId => {
                                                ...
                                                <td className="px-6 py-4 text-sm text-gray-300">
                                                    {butterfly.breeding === 1 ? 
                                                        <div>
                                                            ...
                                                            {Array.isArray(butterfly.breeding_type_id) && butterfly.breeding_type_id.length > 0 && (
                                                                <div className="mt-1 text-xs">
                                                                    <strong>Tipe Aktivitas:</strong> {
                                                                        butterfly.breeding_type_id.map(id => {
                                                                            const types = {1:'Berjemur',2:'Mengisap nektar',3:'Melumpur',4:'Istirahat',5:'Mengisap buah busuk',6:'Kawin',7:'Meletakan telur',16:'Terbang',17:'Menghisap kotoran'};
                                                                            return types[id] || '';
                                                                        }).filter(Boolean).join(', ')
                                                                    }
                                                                </div>
                                ...
                                <div className="space-y-2 mb-2">
                                    ...
                                    {formData.breeding === 1 && (
                                        <div className="mt-3 bg-[#2c2c2c] p-3 rounded-lg">
                                            <p className="text-sm font-medium text-gray-300 mb-2">Pilih tipe aktivitas (bisa pilih lebih dari satu):</p>
                                            <div className="space-y-2">
                                                {[
                                                    { id: 1, label: 'Berjemur' },
                                                    { id: 2, label: 'Mengisap nektar' },
                                                    { id: 3, label: 'Melumpur' },
                                                    { id: 4, label: 'Istirahat' },
                                                    { id: 5, label: 'Mengisap buah busuk' },
                                                    { id: 6, label: 'Kawin' },
                                                    { id: 7, label: 'Meletakan telur' },
                                                    { id: 16, label: 'Terbang' },
                                                    { id: 17, label: 'Menghisap kotoran' },
                                                ].map(type => (
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
                        <div className="bg-[#1e1e1e] p-6 rounded shadow-lg w-96 border border-[#444]">
                            ...
                            <div className="text-gray-300">
                                ...
                                <p><strong className="text-white">Nama Lokasi:</strong> {formData.label || '-'}</p>
                                <p><strong className="text-white">Habitat:</strong> {formData.habitat || '-'}</p>
                                ...
                                <div className="max-h-40 overflow-y-auto mt-2 pr-2">
                                    {butterflyList.map((butterfly, index) => (
                                        <div key={index} className="mt-2 border-t border-[#444] pt-2">
                                            ...
                                            {butterfly.breeding === 1 && Array.isArray(butterfly.breeding_type_id) && butterfly.breeding_type_id.length > 0 && (
