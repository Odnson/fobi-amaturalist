...
const isValidBounds = (bounds) => {
  ...
  return bbox.length === 4 && 
         bbox.every(coord => !isNaN(coord) && isFinite(coord)) &&
         bbox[1] >= -90 && bbox[1] <= 90 && // south
         bbox[3] >= -90 && bbox[3] <= 90;   // north
  // Longitude bisa melebihi -180/180 saat peta di-wrap, itu valid dari Leaflet
};
...
export const useGridData = () => {
  ...
  const updateGridData = useCallback(async (markers, zoomLevel, bounds) => {
    ...
    // Buat fingerprint dari konten markers (bukan hanya jumlah)
    // Ambil sample ID+source dari beberapa markers untuk deteksi perubahan konten
    const sampleSize = Math.min(markers.length, 10);
    const step = markers.length > sampleSize ? Math.floor(markers.length / sampleSize) : 1;
    let fingerprint = '';
    for (let i = 0; i < markers.length && fingerprint.length < 200; i += step) {
      const m = markers[i];
      fingerprint += `${m.id || ''}${m.source || ''},`;
    }
